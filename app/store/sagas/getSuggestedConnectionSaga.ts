...
export default function* getSuggestedConnectionsSaga(action: any) {
  try {
    ...
    if (response.data.statusCode === 200) {
      ...
      if (Array.isArray(response?.data?.body.suggested_users)) {
        yield put(
          appAction.getSuggestedConnectionsResponse(
            response.data.body.suggested_users
          )
        );
      } else {
        yield put(appAction.getSuggestedConnectionsResponse([]));
      }
    } else {
