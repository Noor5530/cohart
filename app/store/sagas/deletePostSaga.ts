...
export default function* newPost(action) {
  try {
    ...
    const userPost: UserState = yield select(
      (state: AppState) => state.app.userPost
    );
    ...
    //how to call api
    const response: Response<Posts[]> = yield call(deletePost, action.data);
    yield put(loadingAction.disableLoading());

    //mock response

    if (response.data.statusCode === 200) {
      const data = userPost?.filter(
        (item) => item?._id !== action.data?.post_id
      );
      yield put(appAction.getUserPostsResponse(data));
    } else {
      throw new Error("");
    }
  } catch (error: any) {
