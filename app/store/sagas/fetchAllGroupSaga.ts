...
export default function* getAllPost(action: any) {
  try {
    yield put(loadingAction.enableAppLoading());

    const response: Response<any[]> = yield call(fetchALLGroup, action.data);
    if (response.data.statusCode === 200) {
      yield put(appAction.getAllUserGroupResponse(response.data.data));
