...
const ManipulatePost = (posts: Posts[]) => {
  const ReferFriendPost = CreateStaticPost(FirstTimeComponents.REFER_FRIEND);

  let manipulatedPost = [...posts];
  if (posts.length >= 7) {
    const listPost1 = manipulatedPost.splice(0, 8);
    const listPost2 = manipulatedPost.splice(8, manipulatedPost.length);
...
export const DiscoverFeed: FC<IProps> = ({ route }: IProps) => {
  ...
  useEffect(() => {
    setAllPosts(ManipulatePost(posts));
  }, [posts]);
          ...
          if (cell) {
            if (item.isViewable) {
              ...
              if (item.startedViewing) {
                ...
                const duration =
                  stoppedViewing.getTime() - item.startedViewing.getTime();
  ...
  const onRefresh = useCallback(
    (postId?: string) => {
      if (userId && userId != 0) {
        ...
        setPageNumber(1);
        setError(false);
        if (postId && posts.length > 0 && allPosts.length > 0 && index > 0) {
          if ((flatListRef as any).current != null) {
        ...
        getAllPosts({
          ...
          .then((response) => {
            if (
              ...
              // const prevCell = videoComponentRef[`id_${index - 1}`];
              // prevCell?.pause();
              // cell?.play();
  ...
  return (
    <>
      <View style={{ flex: 1, backgroundColor: "white" }}>
        ...
        />
        <Tutorial isModalVisible={sowTutorial} toggleModal={setSowTutorial} />
      </View>
    </>
  );
};
